...
const testEnvironment = {
  ...
  INTERNAL_API_KEY: "module7-test-internal-api-key-that-is-at-least-32-characters",
...
/** Runs one child process with the isolated Module 19 backend test environment. */
...
/** Recreates a deterministic database containing every migration and current platform RBAC. */
...
try {
  ...
  // Re-run Foundation and every released prerequisite backend suite against the Module 19 schema.
  ...
  console.log("Module 19 backend verification completed successfully.");
} finally {
  if (started) {
    try {
      await run("docker", ["compose", "-f", composeFile, "down", "-v"]);
    } catch (error) {
      console.error("Failed to stop the Module 19 test infrastructure cleanly:", error);
    }
  }
}
