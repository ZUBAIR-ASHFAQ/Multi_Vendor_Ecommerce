...
const testEnvironment = {
  ...
  INTERNAL_API_KEY: "module11-test-internal-api-key-that-is-at-least-32-characters",
...
/** Runs one child process with the isolated Module 11 backend verification environment. */
...
try {
  ...
  await run(executable("npm"), ["run", "test:module11:migrations"]);
  await prepareDatabase();
  await run(executable("npm"), ["run", "test:module11:specs"]);
  await run(executable("npm"), ["run", "test:regression:contracts"]);

  // Re-run every released prerequisite backend suite against the Module 11 schema.
  for (const script of [
  ...
  console.log("Module 11 Orders backend verification completed successfully.");
} finally {
  if (started) {
    try {
      await run("docker", ["compose", "-f", composeFile, "down", "-v"]);
    } catch (error) {
      console.error("Failed to stop the Module 11 test infrastructure cleanly:", error);
    }
  }
}
