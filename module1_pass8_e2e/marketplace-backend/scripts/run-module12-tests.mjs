...
const testEnvironment = {
  ...
  INTERNAL_API_KEY: "module12-test-internal-api-key-that-is-at-least-32-characters",
  ...
  STRIPE_SECRET_KEY: "sk_test_module12_backend_proof_placeholder",
  STRIPE_WEBHOOK_SECRET: "whsec_module12_backend_proof_placeholder",
...
/** Runs one child process with the isolated Module 12 backend verification environment. */
...
try {
  ...
  await run(executable("npm"), ["run", "test:module12:migrations"]);
  await prepareDatabase();
  await run(executable("npm"), ["run", "test:module12:specs"]);
  await run(executable("npm"), ["run", "test:regression:contracts"]);

  // Re-run every released prerequisite backend suite against the Module 12 schema.
  for (const script of [
  ...
  console.log("Module 12 Payments backend verification completed successfully.");
} finally {
  if (started) {
    try {
      await run("docker", ["compose", "-f", composeFile, "down", "-v"]);
    } catch (error) {
      console.error("Failed to stop the Module 12 test infrastructure cleanly:", error);
    }
  }
}
