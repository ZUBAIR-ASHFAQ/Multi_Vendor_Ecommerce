...
/** Runs one child process with the isolated Module 5 backend test environment. */
...
/** Recreates a deterministic database containing all migrations and the current platform RBAC seed. */
...
try {
  ...
  // Re-run completed prerequisite/released modules against the Module 5 schema before accepting backend work.
  ...
  console.log("Module 5 backend verification completed successfully.");
} finally {
  if (started) {
    try {
      await run("docker", ["compose", "-f", composeFile, "down", "-v"]);
    } catch (error) {
      console.error("Failed to stop the Module 5 test infrastructure cleanly:", error);
    }
  }
}
