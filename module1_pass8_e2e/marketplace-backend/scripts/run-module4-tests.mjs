...
/** Runs one child process with the isolated Module 4 backend test environment. */
...
try {
  ...
  // Re-run completed prerequisite modules against the Module 4 schema before accepting backend work.
  ...
  console.log("Module 4 backend verification completed successfully.");
} finally {
  if (started) {
    try {
      await run("docker", ["compose", "-f", composeFile, "down", "-v"]);
    } catch (error) {
      console.error("Failed to stop the Module 4 test infrastructure cleanly:", error);
    }
  }
}
