...
/** Runs one child process with the isolated Module 21 test environment. */
...
try {
  ...
  console.log("Module 21 backend verification completed successfully.");
} finally {
  if (started) {
    try {
      await run("docker", ["compose", "-f", composeFile, "down", "-v"]);
    } catch (error) {
      console.error("Failed to stop the Module 21 test infrastructure cleanly:", error);
    }
  }
}
