...
const sharedBackendEnvironment = {
  ...process.env,
  NODE_ENV: "test",
  SERVICE_NAME: "marketplace-backend-module5-e2e",
  ...
  DB_APPLICATION_NAME: "marketplace-backend-module5-e2e",
  REDIS_URL: redisUrl,
  TEST_REDIS_URL: redisUrl,
  BULLMQ_PREFIX: "marketplace-module5-e2e",
  JOB_DEFAULT_ATTEMPTS: "2",
  JOB_BACKOFF_MS: "100",
  JOB_DEFAULT_CONCURRENCY: "1",
  JWT_ACCESS_SECRET: "module5-e2e-access-secret-that-is-at-least-32-characters-long",
  JWT_ISSUER: "marketplace-api-module5-e2e",
  JWT_AUDIENCE: "marketplace-web-module5-e2e",
  ...
  STORAGE_BUCKET: "marketplace-module5-e2e",
...
try {
  logStage("Dependency-free Module 2 + Module 21 + Module 3 + Module 4 + Module 5 structure");
  ...
  logStage("Playwright Foundation, Module 2, Module 21, Module 3, Module 4 and Module 5 workflows");
  await run(
    executable("npm"),
    [
      "run",
  ...
  await run("docker", ["build", "-t", "marketplace-backend-module5-release", "."], {
    cwd: backendDirectory,
    env: sharedBackendEnvironment,
  });
  await run(
  ...
  if (composeStarted) {
    try {
      ...
      console.error("Failed to stop Module 5 test infrastructure cleanly:", error);
    }
  }
}
