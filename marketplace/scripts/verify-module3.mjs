...
const sharedBackendEnvironment = {
  ...process.env,
  NODE_ENV: "test",
  SERVICE_NAME: "marketplace-backend-module3-e2e",
  ...
  DB_APPLICATION_NAME: "marketplace-backend-module3-e2e",
  REDIS_URL: redisUrl,
  TEST_REDIS_URL: redisUrl,
  BULLMQ_PREFIX: "marketplace-module3-e2e",
  JOB_DEFAULT_ATTEMPTS: "2",
  JOB_BACKOFF_MS: "100",
  JOB_DEFAULT_CONCURRENCY: "1",
  JWT_ACCESS_SECRET: "module3-e2e-access-secret-that-is-at-least-32-characters-long",
  JWT_ISSUER: "marketplace-api-module3-e2e",
  JWT_AUDIENCE: "marketplace-web-module3-e2e",
  ...
  STORAGE_BUCKET: "marketplace-module3-e2e",
...
/** Fails early with clear setup instructions when project dependencies are absent. */
function requireInstalledDependencies() {
  for (const directory of [backendDirectory, frontendDirectory]) {
    if (!existsSync(path.join(directory, "node_modules"))) {
      throw new Error(
        `Dependencies are not installed in ${path.basename(directory)}. Run npm install independently in both projects first.`,
      );
    }
  }
}
...
function verifyOpenApiPath(document, route, expected) {
  ...
  if (
    ...
    throw new Error(
      `OpenAPI method mismatch for ${route}. Expected ${expectedMethods.join(", ")}; received ${actualMethods.join(", ")}.`,
    );
  }
...
async function verifyLiveImplementedOpenApi() {
  ...
  const module3Paths = new Map([
    ...
    ["/api/v1/customers/me/addresses/{id}", { patch: "updateCurrentCustomerAddress", delete: "archiveCurrentCustomerAddress" }],
    ["/api/v1/admin/customers", { get: "listCustomersForAdmin" }],
    ["/api/v1/admin/customers/{id}", { get: "getCustomerForAdmin" }],
  ]);

  for (const [route, methods] of [...module2Paths, ...module21Paths, ...module3Paths]) {
    verifyOpenApiPath(document, route, methods);
  }

  const actualAuthPaths = Object.keys(document.paths ?? {}).filter((route) => route.startsWith("/api/v1/auth/"));
  const expectedAuthPaths = [...module2Paths.keys()].filter((route) => route.startsWith("/api/v1/auth/"));
  if (
    actualAuthPaths.length !== expectedAuthPaths.length ||
    actualAuthPaths.some((route) => !expectedAuthPaths.includes(route))
  ) {
    throw new Error(`Live OpenAPI Auth paths do not match the approved Module 2 surface: ${actualAuthPaths.join(", ")}`);
  }

  const actualModule21Paths = Object.keys(document.paths ?? {}).filter(
  ...
  if (
    ...
    throw new Error(`Live OpenAPI Module 21 paths do not match the approved surface: ${actualModule21Paths.join(", ")}`);
  }

  const actualModule3Paths = Object.keys(document.paths ?? {}).filter(
...
try {
  logStage("Dependency-free Module 2 + Module 21 + Module 3 structure");
  ...
  await run(executable("npm"), ["run", "test:module3:migrations"], { cwd: backendDirectory, env: sharedBackendEnvironment });
  ...
  logStage("Playwright Foundation, Module 2, Module 21 and Module 3 workflows");
  await run(
    executable("npm"),
    [
      "run",
  ...
  await run("docker", ["build", "-t", "marketplace-backend-module3-release", "."], {
    cwd: backendDirectory,
    env: sharedBackendEnvironment,
  });
  await run(
    ...
    [
      ...
      "VITE_APP_NAME=Marketplace",
      "-t",
      "marketplace-frontend-module3-release",
      ".",
    ],
    { cwd: frontendDirectory, env: frontendEnvironment },
  );
  ...
  if (composeStarted) {
    try {
      ...
      console.error("Failed to stop Module 3 test infrastructure cleanly:", error);
    }
  }
}
