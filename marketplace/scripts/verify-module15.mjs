  ...
  for (const directory of [backendDirectory, frontendDirectory]) {
    for (const required of ["package.json", "node_modules"]) {
      ...
      if (!existsSync(target)) {
        throw new Error(
          `Module 15 release verification requires ${target}. Run npm install independently in both projects first.`,
        );
      }
    }
  }
...
/** Runs the final Module 15 release gate across the independent frontend and backend repositories. */
async function main() {
  logStage("Dependency-free Module 15 structure, HTTP, backend-test, and frontend checks");
  await run("node", [path.join("scripts", "verify-module15-static.mjs")]);
  await run("node", [path.join("scripts", "verify-module15-http.mjs")]);
  ...
  logStage("Module 15 runtime regression, Playwright, reconciliation, build, and container gate");
  await run(executable("npm"), ["run", "test:e2e:ci"], { cwd: frontendDirectory });

  console.log("\nModule 15 Reviews & Ratings full release verification completed successfully.");
}
