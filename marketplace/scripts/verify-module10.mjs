  ...
  for (const directory of [backendDirectory, frontendDirectory]) {
    for (const required of ["package.json", "node_modules"]) {
      ...
      if (!existsSync(target)) {
        throw new Error(
          `Module 10 release verification requires ${target}. Run npm install independently in both projects first.`,
        );
      }
    }
  }
...
/** Runs every dependency-free structural verifier required by the Module 10 dependency chain. */
...
/** Runs the final Module 10 release gate across both independent repositories. */
async function main() {
  logStage("Dependency-free Module 10 structure and dependency chain");
  await runStaticDependencyChain();
  requireInstalledProjects();

  logStage("Backend Module 10 migration, service, API, regression, lint, typecheck, and build gate");
  await run(executable("npm"), ["run", "test:module10"], { cwd: backendDirectory });

  logStage("Frontend Module 10 lint, typecheck, RTL/MSW, and production build gate");
  await run(executable("npm"), ["run", "lint"], { cwd: frontendDirectory });
  await run(executable("npm"), ["run", "typecheck"], { cwd: frontendDirectory });
  await run(executable("npm"), ["run", "test:module10"], { cwd: frontendDirectory });
  await run(executable("npm"), ["run", "build"], { cwd: frontendDirectory });

  logStage("Live OpenAPI, Playwright Module 10, post-browser data integrity, and container gate");
