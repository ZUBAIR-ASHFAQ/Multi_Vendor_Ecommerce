...
const sharedBackendEnvironment = {
  ...process.env,
  NODE_ENV: "test",
  SERVICE_NAME: "marketplace-backend-module9-e2e",
  ...
  DB_APPLICATION_NAME: "marketplace-backend-module9-e2e",
  REDIS_URL: redisUrl,
  TEST_REDIS_URL: redisUrl,
  BULLMQ_PREFIX: "marketplace-module9-e2e",
  JOB_DEFAULT_ATTEMPTS: "2",
  JOB_BACKOFF_MS: "100",
  JOB_DEFAULT_CONCURRENCY: "1",
  JWT_ACCESS_SECRET: "module9-e2e-access-secret-that-is-at-least-32-characters-long",
  INTERNAL_API_KEY: "module9-e2e-internal-api-key-that-is-at-least-32-characters",
  JWT_ISSUER: "marketplace-api-module9-e2e",
  JWT_AUDIENCE: "marketplace-web-module9-e2e",
  ...
  STORAGE_BUCKET: "marketplace-module9-e2e",
...
async function verifyLiveImplementedOpenApi() {
  ...
  const actualModule9Paths = allPaths.filter(
    (route) =>
      route === "/api/v1/admin/promotions" ||
      route.startsWith("/api/v1/admin/promotions/") ||
      route === "/api/v1/seller/promotions" ||
      route === "/api/v1/promotions/validate",
  );
  if (
    actualModule9Paths.length !== module9Paths.size ||
    actualModule9Paths.some((route) => !module9Paths.has(route))
  ) {
    throw new Error(
      `Live OpenAPI Module 9 paths do not match the approved surface: ${actualModule9Paths.join(", ")}`,
    );
  }
...
try {
  logStage("Dependency-free Foundation modules through Module 9 structure");
  ...
  await run("node", ["scripts/verify-module9-static.mjs"]);
  ...
  await run(executable("npm"), ["run", "test:module9:migrations"], {
    cwd: backendDirectory,
    env: sharedBackendEnvironment,
  });
  ...
  logStage("Module 9 backend regression");
  await prepareBackendRegressionDatabase();
  await run(executable("npm"), ["run", "test:module9:specs"], { cwd: backendDirectory, env: sharedBackendEnvironment });
  ...
  await run(executable("npm"), ["run", "test:module9"], { cwd: frontendDirectory, env: frontendEnvironment });
  ...
  logStage("Playwright Foundation through Module 9 Promotions & Coupons workflows");
  await run(
    executable("npm"),
    [
      "run",
  ...
  logStage("Post-E2E database, Product, Inventory, Search, Cart, Wishlist, and Promotion integrity");
  ...
  await run("docker", ["build", "-t", "marketplace-backend-module9-release", "."], {
    cwd: backendDirectory,
    env: sharedBackendEnvironment,
  });
  await run(
    ...
    [
      ...
      "VITE_APP_NAME=Marketplace",
      "-t",
      "marketplace-frontend-module9-release",
      ".",
    ],
    { cwd: frontendDirectory, env: frontendEnvironment },
  );

  console.log("\nModule 9 full release verification completed successfully.");
} finally {
  ...
  if (composeStarted) {
    try {
      ...
      console.error("Failed to stop Module 9 test infrastructure cleanly:", error);
    }
  }
}
