...
const testEnvironment = {
  ...
  INTERNAL_API_KEY: "module9-test-internal-api-key-that-is-at-least-32-characters",
...
/** Runs one child process with the isolated Module 9 backend test environment. */
...
/** Recreates a deterministic database containing every migration and the current platform RBAC seed. */
...
try {
  ...
  await run(executable("npm"), ["run", "test:module9:specs"]);
  await run(executable("npm"), ["run", "test:regression:contracts"]);

  // Re-run Foundation and every released prerequisite backend suite against the Module 9 schema.
  ...
  console.log("Module 9 backend verification completed successfully.");
} finally {
  if (started) {
    try {
      await run("docker", ["compose", "-f", composeFile, "down", "-v"]);
    } catch (error) {
      console.error("Failed to stop the Module 9 test infrastructure cleanly:", error);
    }
  }
}
