...
const testEnvironment = {
  ...
  INTERNAL_API_KEY: "module1-test-internal-api-key-that-is-at-least-32-characters",
  ...
  STRIPE_SECRET_KEY: "sk_test_module1_placeholder",
  STRIPE_WEBHOOK_SECRET: "whsec_module1_placeholder",
...
/** Fails early with a clear setup message when Module 1 runtime prerequisites are missing. */
function verifyRuntimePrerequisites() {
  ...
  if (!existsSync(vitestBinary)) {
    throw new Error("Backend dependencies are not installed. Run `npm ci` before the Module 1 runtime gate.");
  }

  if (!commandExists("docker")) {
    throw new Error("Docker is required for the Module 1 PostgreSQL and Redis runtime tests.");
  }
  if (!commandExists("docker", ["compose", "version"])) {
    throw new Error("Docker Compose v2 is required for the Module 1 runtime tests.");
  }
}

/** Runs one child process with the isolated Module 1 backend verification environment. */
...
/** Recreates a deterministic database with all migrations plus current RBAC and report definitions. */
...
try {
  ...
  console.log("Module 1 Dashboard backend verification completed successfully.");
} finally {
  if (started) {
    try {
      await run("docker", ["compose", "-f", composeFile, "down", "-v"]);
    } catch (error) {
      console.error("Failed to stop the Module 1 test infrastructure cleanly:", error);
    }
  }
}
