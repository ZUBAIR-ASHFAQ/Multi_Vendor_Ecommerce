...
/** Runs one child process with the isolated Module 7 backend test environment. */
...
try {
  ...
  // Re-run Foundation and every released prerequisite backend suite against the Module 7 schema.
  ...
  console.log("Module 7 backend verification completed successfully.");
} finally {
  if (started) {
    try {
      await run("docker", ["compose", "-f", composeFile, "down", "-v"]);
    } catch (error) {
      console.error("Failed to stop the Module 7 test infrastructure cleanly:", error);
    }
  }
}
