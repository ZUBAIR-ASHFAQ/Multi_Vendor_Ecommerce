...
/** Runs one child process with the isolated Module 6 backend test environment. */
...
try {
  ...
  // Re-run Foundation and every released prerequisite backend suite against the Module 6 schema.
  ...
  console.log("Module 6 backend verification completed successfully.");
} finally {
  if (started) {
    try {
      await run("docker", ["compose", "-f", composeFile, "down", "-v"]);
    } catch (error) {
      console.error("Failed to stop the Module 6 test infrastructure cleanly:", error);
    }
  }
}
