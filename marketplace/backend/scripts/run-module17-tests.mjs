...
const testEnvironment = {
  ...
  INTERNAL_API_KEY: "module17-test-internal-api-key-that-is-at-least-32-characters",
  ...
  STRIPE_SECRET_KEY: "sk_test_module17_backend_proof_placeholder",
  STRIPE_WEBHOOK_SECRET: "whsec_module17_backend_proof_placeholder",
...
/** Runs one child process with the isolated Module 17 backend verification environment. */
...
try {
  ...
  await run(executable("npm"), ["run", "test:module17:migrations"]);
  await prepareDatabase();
  await run(executable("npm"), ["run", "test:module17:specs"]);
  await run(executable("npm"), ["run", "test:regression:contracts"]);

  // Re-run every released upstream backend suite against the Module 17 schema and service boundaries.
  for (const script of [
  ...
  console.log("Module 17 Seller Wallet & Payouts backend verification completed successfully.");
} finally {
  if (started) {
    try {
      await run("docker", ["compose", "-f", composeFile, "down", "-v"]);
    } catch (error) {
      console.error("Failed to stop the Module 17 test infrastructure cleanly:", error);
    }
  }
}
